    ...
    public class submit_ajax : IHttpHandler, IRequiresSessionState
    {
        ...
        private void user_register(HttpContext context)
        {
            ...
            else
            {
                ...
                //防止Session提前过期
                Utils.WriteCookie(MXKeys.COOKIE_USER_NAME_REMEMBER, "WeiXinPF", model.user_name);
                Utils.WriteCookie(MXKeys.COOKIE_USER_PWD_REMEMBER, "WeiXinPF", model.password);
        ...
        private void user_login(HttpContext context)
        {
            ...
            {
                Utils.WriteCookie(MXKeys.COOKIE_USER_NAME_REMEMBER, "WeiXinPF", model.user_name, 43200);
                Utils.WriteCookie(MXKeys.COOKIE_USER_PWD_REMEMBER, "WeiXinPF", model.password, 43200);
            }
            else
            {
                //防止Session提前过期
        ...
        private void order_save(HttpContext context)
        {
            ...
            //检查购物车商品
            IList<Model.cart_items> iList = WeiXinPF.Web.UI.ShopCart.GetList(user_group_id);
            if (iList == null)
            {
            ...
            //统计购物车
            Model.cart_total cartModel = WeiXinPF.Web.UI.ShopCart.GetTotal(user_group_id);
            //保存订单=======================================================================
            ...
            //清空购物车
            WeiXinPF.Web.UI.ShopCart.Clear("0");
            //提交成功，返回URL
