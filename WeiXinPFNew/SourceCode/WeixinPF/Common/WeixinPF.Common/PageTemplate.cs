...
namespace WeixinPF.Common
{
    ...
    public abstract class PageTemplate
    {
        ...
        public static string GetTemplate(string sitePath, string tempPath, string skinName, string templet, string fromPage, string inherit, string buildPath, int nest)
        {
            ...
            using (StreamReader objReader = new StreamReader(templetFullPath, Encoding.UTF8))
            {
                ...
                for (int i = 0; i < strlist.Length; i++)
                {
                    if (strlist[i] == "")
                        continue;
                    strReturn.Append(ConvertTags(nest, sitePath, tempPath, skinName, strlist[i])); //搜索替换标签
                }
        ...
        /// <returns></returns>
        private static string ConvertTags(int nest, string sitePath, string tempPath, string skinName, string inputStr)
        {
            string strReturn = "";
            ...
            foreach (Match m in r[0].Matches(strTemplate))
            {
                ...
                {
                    strTemplate = strTemplate.Replace(m.Groups[0].ToString(), "\r\n" + GetTemplate(sitePath, "templates", m.Groups[3].ToString(), m.Groups[4].ToString(), "", "", "", nest + 1) + "\r\n");
                }
                else
                {
                    strTemplate = strTemplate.Replace(m.Groups[0].ToString(), "\r\n" + GetTemplate(sitePath, tempPath, skinName, m.Groups[4].ToString(), "", "", "", nest + 1) + "\r\n");
                }
            }
            #endregion
