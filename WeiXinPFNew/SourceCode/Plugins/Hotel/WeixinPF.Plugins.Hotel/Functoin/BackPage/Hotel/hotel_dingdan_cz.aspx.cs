...
namespace WeixinPF.Hotel.Plugins.Functoin.BackPage.Hotel
{
    public partial class hotel_dingdan_cz : ManagePage
    {
        ...
        public RefundOrderDto RefundOrder = new RefundOrderDto();
...
//                    WHEN 4 THEN ''已退款''
        ...
        protected void save_groupbase_Click(object sender, EventArgs e)
        {
            ...
            {
                OrderId = OrderId,StatusId = status.ToInt()
            };
            var response = BusHelper.Send<OperateIsSuccessResponse>(request);
            if (response.Success)
            {
                AddAdminLog(MXEnums.ActionEnum.Edit.ToString(), "修改酒店状态为" + status + "，主键为" + OrderId); //记录日志
        ...
        protected void btn_completed_OnClick(object sender, EventArgs e)
        {
            ...
            {
                OrderId = OrderId,
                StatusId = HotelStatusManager.OrderStatus.Completed.StatusId
            };
            var response = BusHelper.Send<OperateIsSuccessResponse>(request);
            if (response.Success)
            {
                AddAdminLog(MXEnums.ActionEnum.Edit.ToString(), "修改酒店状态为" +
                   ...
                   + HotelStatusManager.OrderStatus.Completed.StatusId + "，主键为" + OrderId); //记录日志
